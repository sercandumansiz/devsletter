...
export default function DevsLetterHeader() {
    const { register, control, handleSubmit, errors } = useForm<LetterPublishRequest>({
                ...
                <form className="LetterPublishForm" onSubmit={handleSubmit(onSubmit)}>
                    {fields.map((item, index) => (
                        ...
                        defaultValue={item.link}
                            />
                        {errors.items && <p>{errors.items[index]?.link?.message} link is required</p>}
                        </div>
                    ))}
